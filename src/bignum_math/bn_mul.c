...
static bignum *multiply(bignum *n1, bignum *n2)
{
	...
	if (!n1->len || !n2->len)
		...
		return (bignum_alloc(1));

	/*Length of product = length of n1 + length of n2*/
	product = bignum_alloc(n1->len + n2->len);
	current_mul = bignum_alloc(n1->len + n2->len);
...
clean_up:
	current_mul = bignum_free(current_mul);
	if (n2_i < n2->len)
		product = bignum_free(product);
