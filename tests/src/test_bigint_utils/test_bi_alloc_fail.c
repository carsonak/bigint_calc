...
Test(memory_allocation_failure, test_bi_alloc,
	 .description = "bi_alloc() returns NULL on failure", .timeout = 1.0)
{
	cr_assert(zero(ptr, bi_alloc(10)));
}

Test(memory_allocation_failure, test_bi_realloc,
	 .description = "bi_resize() returns NULL on failure", .timeout = 1.0)
{
	bigint *n = NULL;

	cr_assert(zero(ptr, bi_resize(n, 10)));
}

Test(memory_allocation_failure, test_bi_dup,
	 .description = "bi_dup() returns NULL on failure", .timeout = 1.0)
{
