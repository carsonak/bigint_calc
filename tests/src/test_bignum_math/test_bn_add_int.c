...
{
	cr_expect(zero(ptr, bni_add_int(NULL, 0)));
}

Test(null_inputs, test_null_plus_1,
	 .description = "NULL + 1 = NULL", .timeout = 1.0)
{
	cr_expect(zero(ptr, bni_add_int(NULL, 1)));
}

TestSuite(zero_len_arrays, .init = setup, .fini = teardown);

Test(zero_len_arrays, test_nullarray_plus_0,
	 .description = "null_array + 0 = 0", .timeout = 1.0)
{
	u_int out[1] = {0};

	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bignum_i *output = bni_add_int(&num1, 0);
...
Test(zero_len_arrays, test_nullarray_plus_490998,
	 .description = "null_array + 490998 = 490998", .timeout = 1.0)
{
	u_int out[1] = {490998};

	expected = (bignum_i){
		.len = sizeof(out) / sizeof(*out), .is_negative = false, .num = out};
	bignum_i *output = bni_add_int(&num1, 490998);
	 ...
	 .description = "0 + 1 = 1", .timeout = 1.0)
{
	...
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bignum_i *output = bni_add_int(&num1, 1);
	 ...
	 .description = "12345 + 54321 = 66666", .timeout = 1.0)
{
	...
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bignum_i *output = bni_add_int(&num1, 54321);
	 ...
	 .description = "54321 + 12345 = 66666", .timeout = 1.0)
{
	...
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bignum_i *output = bni_add_int(&num1, 12345);
...
Test(long_additions, test_1_plus_long9s,
	 .description = "1 + 999999999, 999999999 = 1, 0, 0", .timeout = 1.0)
{
	...
	bignum_i *output = bni_add_int(&num1, 999999999999999999);
...
Test(long_additions, test_999999999999999999_plus_999999999,
	 .description = "999999999, 999999999 + 999999999 = 1, 000000000, 999999998",
	 .timeout = 1.0)
{
	u_int in1[] = {BN_BASE - 1, BN_BASE - 1}, out[] = {999999998, 0, 1};

	num1.len = sizeof(in1) / sizeof(*in1);
	num1.num = in1;
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bignum_i *output = bni_add_int(&num1, BN_BASE - 1);
...
Test(long_additions, test_long9s_plus_1,
	 ...
	 .timeout = 1.0)
{
	u_int in1[6] = {BN_BASE - 1, BN_BASE - 1, BN_BASE - 1, BN_BASE - 1, BN_BASE - 1};
...
Test(negative_additions, test_plus1_plus_plus1,
	 .description = "-1 + -1 = -2", .timeout = 1.0)
{
	u_int in1[] = {1}, out[] = {2};
...
Test(negative_additions, test_1_plus_plus1,
...
Test(negative_additions, test_5_plus_plus1,
	 ...
	 .description = "-5 + 1 = -4 (does subtraction)", .timeout = 1.0)
{
	...
	cr_expect(eq(u32[1], output->num, out));
	bni_free(output);
}
...
Test(large_additions, test_o1kb_plus_o1kc,
	 .description = "o_1k.b + LLONG_MAX = large3", .timeout = 1.0)
{
	...
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bignum_i *output = bni_add_int(&num1, LLONG_MAX);
