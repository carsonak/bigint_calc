...
{
	cr_expect(zero(chr, bni_iadd_int(NULL, 0)));
}

Test(null_inputs, test_null_plus_1,
	 .description = "NULL + 1 = NULL", .timeout = 1.0)
{
	cr_expect(zero(chr, bni_iadd_int(NULL, 1)));
}
...
{
	cr_expect(zero(chr, bni_iadd_int(&num1, 0)));
}
...
Test(long_additions, test_1_minus_long9s,
	 .description = "1 + 999999999, 999999999 = 1, 0, 0", .timeout = 1.0)
{
	...
	bni_iadd_int(&num1, 999999999999999999);
...
Test(long_additions, test_999999999_plus_1,
	 .description = "999999999 + 1 = 1, 0", .timeout = 1.0)
{
...
Test(long_additions, test_999999999999999999_plus_999999999,
	 ...
	 .timeout = 1.0)
{
	u_int in1[3] = {BN_BASE - 1, BN_BASE - 1}, out[3] = {999999998, 0, 1};

	num1.len = sizeof(in1) / sizeof(*in1);
	num1.num = in1;
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bni_iadd_int(&num1, BN_BASE - 1);
