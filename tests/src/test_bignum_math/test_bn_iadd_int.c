...
{
	cr_expect(zero(chr, bn_addint_inplace(NULL, 0)));
}

Test(null_inputs, test_null_plus_1,
	 .description = "NULL + 1 = NULL", .timeout = 2.0)
{
	cr_expect(zero(chr, bn_addint_inplace(NULL, 1)));
}
...
{
	cr_expect(zero(chr, bn_addint_inplace(&num1, 0)));
}
	 ...
	 .description = "1 + 999999999, 999999999 = 1, 0, 0", .timeout = 2.0)
{
	...
	bn_addint_inplace(&num1, 999999999999999999);
...
Test(long_additions, test_999999999_plus_1,
	 .description = "999999999 + 1 = 1, 0", .timeout = 2.0)
{
	 ...
	 .timeout = 2.0)
{
	...
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bn_addint_inplace(&num1, BN_BASE - 1);
	 ...
	 .description = "o_1k.b + LLONG_MAX = large3", .timeout = 2.0)
{
	...
	expected.len = sizeof(out) / sizeof(*out);
	expected.num = out;
	bn_addint_inplace(&num1, LLONG_MAX);
