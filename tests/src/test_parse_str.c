...
{
	cr_assert(zero(ptr, parse_str(NULL)));
}

Test(null_inputs, test_null, .description = "\0001", .timeout = 3.0)
{
	cr_assert(zero(ptr, parse_str("\0001")));
}
	 ...
	 .description = "invalid chars", .timeout = 3.0)
{
	...
	{
		if (!isalnum(c[0]))
			cr_assert(zero(ptr, parse_str(c)));
	}
}
...
{
	cr_redirect_stderr();
	cr_assert(zero(ptr, parse_str("---___")));
	cr_assert(zero(ptr, parse_str("---,,,")));
	cr_assert(zero(ptr, parse_str("+++___,,,")));
	cr_assert(zero(ptr, parse_str("+++_,_,_,")));
	cr_assert(zero(ptr, parse_str("-+-+-+-.")));
	cr_assert(zero(ptr, parse_str("+-+-+-+.")));
	cr_assert(zero(ptr, parse_str("---___.")));
	cr_assert(zero(ptr, parse_str("---,,,.")));
...
{
	cr_redirect_stderr();
	cr_assert(zero(ptr, parse_str("123-567")));
	cr_assert(zero(ptr, parse_str("123 567")));
	cr_assert(zero(ptr, parse_str("123\"567\"")));
	cr_assert(zero(ptr, parse_str("123+567")));
	cr_assert(zero(ptr, parse_str("123/567")));
...
{
	cr_redirect_stderr();
	cr_assert(zero(ptr, parse_str(",20")));
}
...
{
	output = parse_str("0000000000000");
...
{
	output = parse_str("00000000000001");
...
{
	output = parse_str("---10_000_000_000");
...
{
	set_base(36);
	output = parse_str("--ZZZ,4Ar,YU8,012,qa9");
