...
{
	CHECK(bi_isubtract(NULL, NULL) == 0, "");
}
...
TEST(null_inputs, test_1_minus_null)
{
	...
	bi_isubtract(&num1, NULL);

	CHECK(num1.len == expected.len, "length should be %zu", expected.len);
	CHECK(
		num1.is_negative == expected.is_negative, "negative flag should be %u",
		expected.is_negative);
	CHECK_BUF_EQ(
		num1.num, expected.num, expected.len * sizeof(*expected.num), "");
}
...
TEST(null_inputs, test_null_minus_1)
{
	...
	CHECK(bi_isubtract(NULL, &num2) == 0, "");
}
...
{
	CHECK(bi_isubtract(&num1, &num2) == 0, "");
}
...
TEST(zero_len_arrays, test_4490998_minus_nullarray)
{
	...
	num1.num = in1;
	expected = (bigint){
		.is_negative = false, .len = sizeof(out) / sizeof(*out), .num = out};

	bi_isubtract(&num1, &num2);
...
TEST_F_SETUP(negations)
{
	tau->num1 = (bigint){0};
	tau->expected = (bigint){0};
}
TEST_F_TEARDOWN(negations) {}

TEST(negations, test_4490998_minus_NULL)
{
	u_int in1[] = {4490998}, out[] = {4490998};

	num1 = (bigint){
		.is_negative = false, .len = sizeof(in1) / sizeof(*in1), .num = in1};
	expected = (bigint){
		.is_negative = true, .len = sizeof(out) / sizeof(*out), .num = out};
...
TEST(negations, test_neg_4490998_minus_NULL)
{
	u_int in1[] = {4490998}, out[] = {4490998};

	num1 = (bigint){
		.is_negative = true, .len = sizeof(in1) / sizeof(*in1), .num = in1};
	expected = (bigint){
		.is_negative = false, .len = sizeof(out) / sizeof(*out), .num = out};
...
TEST(null_inputs, test_null_minus_null)
{
	CHECK(bi_isubtract_int(NULL, 0) == 0, "");
}

TEST(null_inputs, test_null_minus_1)
{
	CHECK(bi_isubtract_int(NULL, 1) == 0, "");
}
...
TEST(zero_len_arrays, test_nullarray_minus_nullarray)
{
	CHECK(bi_isubtract_int(&num1, 0) == 0, "");
}

TEST(zero_len_arrays, test_nullarray_minus_largenum)
{
	CHECK(bi_isubtract_int(&num1, 490998) == 0, "");
}
...
TEST(simple_subtractions, test_50000_minus_100000000)
{
	...
	bi_isubtract_int(&num1, 1000000000);
...
TEST(same_number_subtractions, test_1000000001_minus_1000000001)
{
	...
	bi_isubtract_int(&num1, 1000000001);
...
TEST(same_number_subtractions, test_longnum_minus_longnum)
{
	...
	bi_isubtract_int(&num1, 1000000001000000001);
...
TEST(long_subtractions, test_1_minus_long9s)
{
	...
	bi_isubtract_int(&num1, 999999999999999999);
...
TEST(long_subtractions, test_4000000000678_minus_999999000)
{
	...
	bi_isubtract_int(&num1, 999999000);
...
TEST(long_subtractions, test_999999000_minus_4000000000678)
{
	...
	bi_isubtract_int(&num1, 4000000000678);
...
TEST(large_subtractions, test_o1kb_minus_o1kc)
{
	...
	bi_isubtract_int(&num1, LLONG_MAX);
