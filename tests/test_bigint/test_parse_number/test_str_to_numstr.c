 ...
 */
void teardown(void) { output = free_numstr(output); }
...
TEST_F(invalid_chars_in_str, test_valid_chars_with_dash_in_middle)
{
	...
	EXPECT(output->len, sizeof(expected) - 1 ==, "");
	EXPECT(processed == 3, "");
	EXPECT(output->is_negative == false, "");
...
TEST_F(invalid_chars_in_str, test_valid_chars_with_space_in_middle)
{
	...
	output = str_to_numstr(in, 10, &processed);
	EXPECT(output->len, sizeof(expected) - 1 ==, "");
	EXPECT(processed, sizeof(expected) - 1 ==, "");
	EXPECT(output->is_negative == false, "");
	EXPECT(output->str == expected, "");
}
...
TEST_F(valid_inputs, test_neg0000000000000)
{
	...
	EXPECT(output->len, sizeof(expected) - 1 ==, "");
	EXPECT(processed, sizeof(in) - 1 ==, "");
	EXPECT(output->is_negative == true, "");
	EXPECT(output->str == expected, "");
}
